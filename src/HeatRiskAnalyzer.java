...
/**
 * Purpose:
 * Performs the main analysis of weather data by filtering records, calculating heat index metrics, and producing summarized results.
 * Demonstrates polymorphism by running multiple Metric computations on the same dataset.
 */
...
public class HeatRiskAnalyzer {
     ...
     */
    public HeatSummary summarize(List<WeatherRecord> dayList,
                                 double threshold,
                                 boolean usePercentile) {

        HeatSummary summary = new HeatSummary();
        summary.totalDays = dayList.size();
        // skippedRowCount is tracked at file load time; surface separately in UI
